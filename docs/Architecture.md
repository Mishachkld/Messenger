...
## SignalR (опционально, для резюме)

Основной канал доставки сообщений (протокол `{type, payload}` поверх WSS, описанный выше) SignalR не использует и не затрагивает — решение не менять его в силе, ровно чтобы переезд `realtime` на Go/Rust оставался переездом одного маленького сервиса, а не переписыванием чужого протокола (SignalR Hub Protocol — задокументирован и переносим в другой язык, есть даже готовая Go-реализация `philippseith/signalr`, но это отдельная, более тяжёлая задача, а не цель этого проекта).

Если нужен реальный опыт с SignalR для резюме, он добавляется как **отдельная, второстепенная фича, которая никогда не мигрирует** — и живёт не в `backend/realtime` (который под нож на этапе 6), а прямо в `backend/api` (сервис постоянный, его код и так остаётся навсегда).

Кандидат-фича: **живой admin-дашборд** — отдельная страница `/admin`, куда сервер сам пушит метрики (сколько пользователей онлайн, сообщений в минуту, список активных чатов):

- `Hub` (`AdminDashboardHub`) живёт в `backend/api`, регистрируется как обычно (`AddSignalR()`, `MapHub<AdminDashboardHub>("/hubs/admin")`).
- Доступ — тот же JWT, что и везде, плюс проверка роли/клейма `admin` (можно захардкодить одного пользователя-админа, полноценный RBAC не нужен).
- Данные для пуша API и так может прочитать: presence-ключи и счётчики в Redis — те же самые, что пишет `realtime`. Фоновый `IHostedService` читает Redis раз в N секунд и рассылает через `IHubContext<AdminDashboardHub>.Clients.Group("admins").SendAsync(...)`.
- Фронт — отдельная маленькая страница с официальным npm-клиентом `@microsoft/signalr` (не самописный WS-клиент). Это специально: получаешь сравнение вживую — свой WS-клиент с ручным reconnect/backoff (чат) vs. официальный SignalR-клиент со встроенным reconnect (admin-дашборд). Для собеседования это весомее, чем «читал про SignalR».

Что это осознанно **не** меняет: протокол чата, `backend/realtime`, план миграции на Go/Rust — не тронуты вообще. Это отдельная ветка, необязательная для прохождения остальных этапов — можно сделать в любой момент после этапа 1 (нужен только рабочий auth), параллельно с основным треком.
...
С определённого этапа перед системой встаёт **nginx** — сначала как reverse proxy (единая точка входа: `/` → frontend, `/api/*` → api, `/ws` → realtime; снаружи один порт и один origin — пропадает CORS), затем как балансировщик нескольких инстансов `api` и `realtime` (`upstream` + `least_conn`).

Почему архитектура это уже позволяет:

- `api` stateless (JWT, состояние в Postgres/Redis) — балансируется тривиально, sticky-сессии не нужны.
- `realtime` держит состояние (открытые сокеты), но каждый инстанс подписан на весь канал `chat-events` и доставляет события только своим сокетам — поэтому неважно, на какой инстанс попало WS-соединение. Один пользователь с двумя вкладками может сидеть на разных инстансах.
- Для проксирования WebSocket в nginx обязательны заголовки `Upgrade`/`Connection` и увеличенный `proxy_read_timeout`.

## CI/CD

GitHub Actions, пайплайн на каждый push/PR:

```
backend-test ─┐
frontend-test ─┼─► docker-build ─► deploy (только ветка main)
go-test ──────┘
```

- `*-test` джобы гоняются параллельно: `dotnet test` (xUnit + Testcontainers для интеграционных тестов с реальным Postgres/Redis в контейнере), `npm run lint && npm run test` (Vitest), `go test ./...` (после этапа рерайта на Go).
- `docker-build` (`needs: [...все test-джобы]`) собирает образы всех сервисов — не запускается, если хоть один тест красный.
- `deploy` (`needs: docker-build`, только на `main`) — это и есть gate: физически не может стартовать раньше зелёных тестов. Публикация образов в `ghcr.io` и/или `docker compose pull && up -d` на сервере по SSH.
- Branch protection на `main` (require status checks) — без этого правила пайплайн просто дашборд, а не то, что реально блокирует влитие сломанного кода.

## docker-compose

Сервисы: `postgres`, `redis`, `api`, `realtime`, `frontend` (dev — Vite, прод — nginx), позже `nginx` (reverse proxy / балансировщик). Масштабирование — `docker compose up --scale api=3 --scale realtime=2`. На этапе перехода на Go в compose меняется только образ `realtime`.
...
6. **Nginx**: сначала reverse proxy (одна точка входа), затем балансировка нескольких инстансов `api` и `realtime`.
7. **CI/CD**: тесты (юнит + интеграционные через Testcontainers) на каждом предыдущем этапе, в конце — пайплайн GitHub Actions с gate на деплой.

## Что осознанно за скоупом

E2E-шифрование, вложения/файлы, доставка на несколько устройств с синхронизацией, push-уведомления, оркестрация сложнее docker-compose (Kubernetes).
